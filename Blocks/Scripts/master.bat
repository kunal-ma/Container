...
:: 1.06 : Automated retrival, assignment and display 
:: of IP adddress in Sector 2A for initiating wireless 
:: connection with a device.

:: 1.10 : Made the big jump with multiple changes and 
:: integrations throughout the program. Discontinued the
:: Hotspot Config and GodMode Registry due to inactivity
:: and to make space for new links & modules. Integrated
:: Croc to facilitate big file transfers to another device
:: over a shared local network without any installation.
:: Added links to Tiles Modifier & HWMonitor for easy
:: access. Removed alert for USB connectivity while 
:: stablishing wireless connection to Android Device.

::					/----------------\
::					:      Bugs      :
::					\----------------/

:: 12 FEB : No input in selections results to unscripted
:: selection of modules, despite 'goto' loop for the 
:: UI components. It can safely be assumed that the bug
:: is due to no value in variable used during the initial
:: run of the script and uses values set during first run
:: to substitue the empty variable in the future runs.
:: No fix can be seen as for now, as beforehand variable
:: declaration is overwritten by empty user input.

:: 13 MAR : Reliance on key word "DHCP" in automation of
:: IP address retrieval in Sector 2A may fail in case of
:: multiple DHCP addresses in IPCONFIG, although this is
:: yet to be seen and re-created practically.
...
:: --------------------------------------------------------
:: Sector 2A : Wireless connection setup
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Module 3 : Administrator Account
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Sector 5B : Croc - Receiving Files
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Module 1 : Hotspot Configuration (Function 4)
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Sector 1A : Configuration data (Function 4)
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Module 5 : Web Server (Discontinued)
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Sector 5A : Comamands for Web Server (Discontinued)
:: --------------------------------------------------------
...
:: --------------------------------------------------------
:: Sector 4A : GodMode Folder Generation
:: --------------------------------------------------------
